...
| `resilience` | **친절형은 항상 null.** 압박 구간이 없어 산출 불가. 압박형이어도 꼬리질문이 하나도 없으면 null |
| `company_comment` | `company_profile_override`가 null이면 null. `company_id`는 보지 않는다. 코멘트 생성에 실패해도 null |
| `improved_answers` | 내용 점수가 낮은 문항 최대 2개. 모든 문항이 잘 답했거나 생성에 실패하면 빈 배열 |

**글로 된 칸(근거 · 개선 답변 · 기업 코멘트)은 생성에 실패해도 리포트를 막지 않는다.**
점수는 이미 나와 있고 이 칸들은 설명이라, 실패하면 빈 배열이나 null로 둔다.
가짜 문장으로 채우지 않는다.

**`improved_answers[].original_excerpt`는 답변 전사에 실제로 있는 구절이다.**
AI가 전사에서 찾지 못한 발췌는 내보내지 않는다.
`t_start`·`t_end`는 그 구절이 말해진 구간이다(해당 답변 오디오 기준 초).
...
압박형 면접에서 **꼬리질문 답변의 내용 점수가 주질문보다 얼마나 떨어졌나**로 본다.

```
하락  = 주질문 내용 점수 평균 - 꼬리질문 내용 점수 평균   (음수면 0)
점수  = 100 - 하락 × 2                                  (0~100)
```

파고드는 질문을 받았을 때 근거를 이어가는 힘을 본다. 이미 있는 내용 점수로
계산하므로 추가 요금이 없고, 같은 답변이면 늘 같은 값이 나온다.
`comment`에는 두 평균과 하락 폭이 들어간다.
